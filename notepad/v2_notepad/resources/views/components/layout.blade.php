            ...
            <div class="flex h-16 items-center justify-between">
                ...
                <div class="hidden md:block">
                    <div class="ml-10 flex items-baseline space-x-4">
                        ...
                        <x-nav-link href="/notepad" :active="request()->is('/notepad')">Notepads</x-nav-link>
                        <x-nav-link href="/configuration" :active="request()->is('/configuration')">Configuration</x-nav-link>
                    </div>
                </div>
                </div>
                <div class="hidden md:block">
                <div class="ml-4 flex items-center md:ml-6">
                    @guest
                    <x-button-second href="/register" :active="request()->is('register')" class="text-white">Register</x-button-second>
                        <x-button-first href="/login" :active="request()->is('login')" class="ml-4">Log In</x-button-first>
                    @endguest

                    @auth
                        <form method="POST" action="/logout">
                            @csrf
                            <x-button-first :isButton="true" type="submit">Log Out</x-button-first>
                        </form>
                    @endauth
                </div>
                </div>
